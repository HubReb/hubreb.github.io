...
Segfaults. Silent memory corruption. Values that looked right in the debugger but weren't. The classic LP64 trap: `sizeof(long)` changes from 4 to 8, every pointer-to-int cast breaks, struct packing shifts, binary formats become unreadable. The codebase compiled cleanly on 64-bit. Then it destroyed itself the moment it ran.
...
The previous team asked: "How do we make this code 64-bit?" That's the right question at the wrong time. The system has to survive January 2038 first. Both problems are rooted in 32-bit — but one is far simpler than the other.
...
**The previous attempt wasn't wrong — it was solving the right problem at the wrong time.** Full 64-bit is the correct long-term solution. It's also a system-wide earthquake on a 30-year codebase with no tests. `_TIME_BITS=64` buys a decade (unsigned timestamps safe until 2106) at a fraction of the cost and risk. Do the surgery you need, not the surgery that's technically ideal.

**Classify before you count.** ~60 warnings look manageable. A handful of them are silent stack corruption. Without classification, you either panic at the number or miss the ones that matter.

**Follow the data end-to-end.** The DB strategy changed three times as I traced each layer deeper: "a couple of tables need changes" → "dozens" → "over a thousand" → "but only a handful are critical, and the bottleneck is one C function." Each revision came from going one layer further.

As engineers, we want to do things right. Sometimes you have to accept doing them ugly to keep going. If you find a hole in your boat, you fix it properly once you're safe on land. As long as you're on the sea? Grab wood, grab nails, and close the hole — no matter how much it hurts your soul.

---

*I migrate decades-old systems to modern stacks — one script, one table, one timestamp at a time. When the previous attempt says "impossible," I check if it was the right question. If that sounds familiar, let's talk.*
