...
A 30+ year old system. 32-bit C, dozens of executables, no tests. The team had tried migrating to 64-bit. The code compiled cleanly. Then it crashed at runtime.
...
Every struct containing these types changes layout. Every `memcpy` using `sizeof(long)` copies the wrong number of bytes. Every binary file format containing these fields becomes unreadable. Every pointer stored as an integer type gets truncated when cast back.
...
This created a false sense of security. The code compiled. Most of it worked. But thousands of occurrences of the four unsafe types scattered across the codebase were silently corrupting memory wherever struct layout or data size mattered.
...
Code that uses `sizeof(long)` where it means "4 bytes." On LP64, `sizeof(long)` is 8. Buffer overflows in `memcpy`. Wrong byte counts in database bind operations. Index structures with hardcoded sizes. Each one a potential data corruption site.
...
The codebase wasn't broken. It was 99% clean with a 1% poison distributed across thousands of occurrences of four types. The fix is mechanical once you know which types to change. The hard part was figuring out which were dangerous and which were safe.

"It compiled" is not "it works." On LP64, the most dangerous bugs are the ones the compiler accepts without complaint. Type classification before migration isn't optional. It's the difference between "too much effort" and "6 weeks."

---

*I migrate decades-old systems to modern stacks — one script, one table, one timestamp at a time. I find the 4 lines that break the other 40,000. If your migration compiled and crashed, let's talk.*
